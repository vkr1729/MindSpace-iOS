...
## Fix Log (Batch 3 — P1 sync/catalog/reliability)
| ID | Fix summary | Test(s) | Commit SHA |
|----|-------------|---------|------------|
| P1-5 | Explicit freeze decision: catalog stays build-bundled (sidecar trusted only on hash match), but `schemaVersion` is now honored — mismatch surfaces a loud `CatalogLoadError.schemaMismatch` ("update the app") instead of silent 404s/hash failures downstream. | `P1Batch3RemediationTests.testP1_5_SchemaMismatchMessage` | _to fill after push_ |
| P1-6 | Streaming now uses the Contents API URL form (`api.github.com/…/contents/…` + `Accept: application/vnd.github.raw`) so the `Authorization` header reaches the bytes directly instead of being dropped across `raw.githubusercontent.com` redirects. | `testP1_6_ContentsAPIURLFormIsWellFormed`, `testP1_6_RawHostRedirectIsTheKnownRisk` | _to fill after push_ |
| P1-7 | Pass consumption is now persisted: `OrbitStats.passProtectedDayKeys` reports which day-keys a pass protected; `CompassionPassStore.reconcile` (hooked into `ProgressDashboardView` appear + event changes) decrements `compassionPassCount` and stamps `lastUsedCompassionPassDate` so used passes can't regenerate. | `testP1_7_NewlyProtectedKeysDriveConsumption` | _to fill after push_ |
| P1-9 | Initial resume seek defers until `status == .readyToPlay` (`pendingInitialSeekSeconds` applied in the observer, cleared on track switch/stop); no more silent start-at-0. | `testP1_9_MissingMediaStillFailsFastWithoutSeek` | _to fill after push_ |
| P1-10 | Per-file 3× retry with exponential backoff for retryable failures (flaky network, 408/429/5xx); verification failures and cancels don't retry; progress counts attempts (not successes); per-file error list (`fileResults`/`failedFiles`) with a named summary instead of one last-error string. `verifyAndMoveFile` (throws→Bool, false-on-hash-mismatch) replaced by `verifyAndMoveFailure` (nil-on-success, typed failure). | `testP1_10_FailureClassification`, `testP1_10_FileResultIdentity` | _to fill after push_ |
| P1-11 | `*.ipa` + `*.ipa.sha256` added to `.gitignore`; all 8 tracked IPAs untracked (`git rm --cached`, files stay on disk). CI remains the only IPA producer (upload-artifact). | Pre-flight: `git ls-files` shows no `.ipa` | _to fill after push_ |
| P1-12 | Honest copy ("Private by Design" + optional-sync disclosure naming both GitHub hosts); `ZeroNetworkTests` rewritten from theater to three real assertions: no third-party telemetry SDKs, networking confined to the sync path, sync touches only first-party GitHub hosts. | Rewritten `ZeroNetworkTests` (3 tests) + `testP1_12_PrivacyCopyMentionsOptionalSync` | _to fill after push_ |
