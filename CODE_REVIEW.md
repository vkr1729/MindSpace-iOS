...
## Fix Log (Batch 2 → Run 6 — P1 completion/data pipeline)
| ID | Fix summary | Test(s) | Commit SHA |
|----|-------------|---------|------------|
| P1-1 | Adopted `VersionedSchema`: `MindSpaceSchemaV1` (4 entities) → `MindSpaceSchemaV1_1` (+ `PendingCompletion` outbox) with `MindSpaceMigrationPlan` (lightweight v1→v1.1). `MindSpaceApp` builds all containers from the versioned schema with the plan; the corrupt-store salvage path now exports-first semantics (existing file moved aside untouched, shm/wal removed) with honest "started fresh, old data kept" banner copy replacing the false "recovered from backup" claim. Removed decorative `StoreSchema.version`. | `P1Batch2RemediationTests.testP1_1_MigrationPlanCoversV1ToV1_1` + `testP1_1_V1_1ContainerRoundTrip` | _to fill after push_ |
| P1-2 | Completion persistence now stamps one `stampedAt` per finalize and flips `isPersisted` on `lastCompletionInfo` via `markLastCompletionPersisted` after the write lands; `CompletionView` reflection UI gates on the persisted event and shows a "still saving" state otherwise; removed the nil-id fallback that could attach reflections to another session's event. | `testP1_2_ReflectionByExactIdOnly`; existing Scenario 6 still passes | _to fill after push_ |
| P1-3 | New persistent `PendingCompletion` outbox: enqueue after the 0.5 s retry fails, flush on launch/foreground/background with duplicate-safe replay (`flushPendingCompletions` returns remaining count); honest banner copy replaces the false "will retry on next launch" claim; resume-failure copy no longer implies silent loss. | `testP1_3_OutboxFlushReplaysQueuedCompletion` + `testP1_3_OutboxFlushSkipsDuplicatesWithoutLoss` | _to fill after push_ |
| P1-4 | Both Files/share/AirDrop entry points (`BackupImportView.onAppear`, `SettingsView.handlePickedDocument`) now use `parseBackupDocument(gainingAccessTo:)` — start/stopAccessing + temp-file copy + parse; real errors surface instead of `try?`. In-app picker path unchanged (already `asCopy`). | `testP1_4_ScopedImportParsesBackupCopy` | _to fill after push_ |
| P1-8 | Completion presentation moved to app level: `ContentView` sheet driven by `lastCompletionInfo` with `acknowledgeLastCompletion` ack (fires for background/lock-screen/MiniPlayer-X completions); player-local sheet kept only when the fullscreen player is presented; `finalizedByStopOrSwitch` distinguishes "Session Saved" (early wrap-up) from natural finish. | `testP1_8_CompletionInfoDefaultsToNaturalFinish` | _to fill after push_ |
