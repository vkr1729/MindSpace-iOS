...
## Fix Log (Batch 2 → Run 11 — P1 completion/data pipeline; CI runs consumed: 11/25)
| ID | Fix summary | Test(s) | Commit SHA |
|----|-------------|---------|------------|
| P1-1 | Adopted `VersionedSchema`: `MindSpaceSchemaV1` (4 entities) → `MindSpaceSchemaV1_1` (+ `PendingCompletion` outbox) with `MindSpaceMigrationPlan` (lightweight v1→v1.1). `MindSpaceApp` builds all containers from one `Schema(versionedSchema:)` value via the `init(for: Schema, migrationPlan, configurations)` overload; the corrupt-store salvage path preserves the old store file and the banner now honestly says "started fresh, old data kept". Removed decorative `StoreSchema.version`. | `P1Batch2RemediationTests.testP1_1_MigrationPlanCoversV1ToV1_1` + `testP1_1_V1_1ContainerRoundTrip` | `696d480` (+ repairs `38e9b24`) |
| P1-2 | Completion persistence now stamps one `stampedAt` per finalize and flips `isPersisted` on `lastCompletionInfo` via `markLastCompletionPersisted` after the write lands; `CompletionView` reflection UI gates on the persisted event and shows a "still saving" state otherwise; removed the nil-id fallback that could attach reflections to another session's event. | `testP1_2_ReflectionByExactIdOnly`; existing Scenario 6 still passes | `696d480` |
| P1-3 | New persistent `PendingCompletion` outbox: enqueue after the 0.5 s retry fails, flush on launch/foreground/background with duplicate-safe replay (`flushPendingCompletions` returns remaining count); honest banner copy replaces the false "will retry on next launch" claim; resume-failure copy no longer implies silent loss. | `testP1_3_OutboxFlushReplaysQueuedCompletion` + `testP1_3_OutboxFlushSkipsDuplicatesWithoutLoss` | `696d480` (+ repairs `4f0bbcd`) |
| P1-4 | Both Files/share/AirDrop entry points (`BackupImportView.onAppear`, `SettingsView.handlePickedDocument`) now use `parseBackupDocument(gainingAccessTo:)` — start/stopAccessing + temp-file copy + parse; real errors surface instead of `try?`. In-app picker path unchanged (already `asCopy`). | `testP1_4_ScopedImportParsesBackupCopy` | `696d480` |
| P1-8 | Completion presentation moved to app level: `ContentView` sheet driven by `lastCompletionInfo` mirrored into a `@State` item with `acknowledgeLastCompletion` ack (fires for background/lock-screen/MiniPlayer-X completions); player-local sheet kept only when the fullscreen player is presented; `finalizedByStopOrSwitch` distinguishes "Session Saved" (early wrap-up) from natural finish. Body split into `rootZStack` + `completionSheet(info:)` helpers for the type-checker; `PlaybackCompletionInfo` is now `Identifiable` (by `completionId`). | `testP1_8_CompletionInfoDefaultsToNaturalFinish` | `696d480` (+ repairs `68625c1`, `9b2ec2c`, `f06ab12`) |

### Batch 2 red-run repairs (all green in Run 11 `35057216752`)
| Run | Failure | Fix | Commit SHA |
|-----|---------|-----|------------|
| Run 6 `35056522934` | `ModelContainer(for: MindSpaceSchemaV1_1.self, …)` matches no initializer; `versionIdentifier var` flagged under complete concurrency | Build from one `Schema(versionedSchema:)` via the `Schema` overload; `let` identifiers | `38e9b24` |
| Run 7 `35056722499` | `try await` inside `XCTAssert` autoclosure (no concurrency support) | Hoist actor results into locals, then assert | `4f0bbcd` |
| Run 8 `35056841228` | Body type-check timeout from inline `Binding(get:set:)` + sheet content | Hoist sheet item to `@State` + `onChange` mirror | `68625c1` |
| Run 9 `35056969845` | Body still too complex (ZStack + all modifiers in one expression) | Split into `rootZStack` + `completionSheet(info:)` helpers | `9b2ec2c` |
| Run 10 `35057110554` | `.sheet(item:)` requires `Identifiable` | `PlaybackCompletionInfo: Identifiable` via `completionId` | `f06ab12` |
| Run 11 `35057216752` | **SUCCESS** — full gate green (tests + unsigned build + IPA package) | — | `f06ab12` |

## Fix Log (Batch 3 — P1 sync/catalog/reliability; CI runs consumed: 13/25)
| ID | Fix summary | Test(s) | Commit SHA |
|----|-------------|---------|------------|
| P1-5 | Explicit freeze decision: catalog stays build-bundled (sidecar trusted only on hash match), but `schemaVersion` is now honored — mismatch surfaces a loud `CatalogLoadError.schemaMismatch` ("update the app") instead of silent 404s/hash failures downstream. | `P1Batch3RemediationTests.testP1_5_SchemaMismatchMessage` | `a3fa10a` |
| P1-6 | Streaming now uses the Contents API URL form (`api.github.com/…/contents/…` + `Accept: application/vnd.github.raw`) so the `Authorization` header reaches the bytes directly instead of being dropped across `raw.githubusercontent.com` redirects. | `testP1_6_ContentsAPIURLFormIsWellFormed`, `testP1_6_RawHostRedirectIsTheKnownRisk` | `a3fa10a` |
| P1-7 | Pass consumption is now persisted: `OrbitStats.passProtectedDayKeys` reports which day-keys a pass protected; `CompassionPassStore.reconcile` (hooked into `ProgressDashboardView` appear + event changes) decrements `compassionPassCount` and stamps `lastUsedCompassionPassDate` so used passes can't regenerate. | `testP1_7_NewlyProtectedKeysDriveConsumption` | `a3fa10a` |
| P1-9 | Initial resume seek defers until `status == .readyToPlay` (`pendingInitialSeekSeconds` applied in the observer, cleared on track switch/stop); no more silent start-at-0. | `testP1_9_MissingMediaStillFailsFastWithoutSeek` | `a3fa10a` |
| P1-10 | Per-file 3× retry with exponential backoff for retryable failures (flaky network, 408/429/5xx); verification failures and cancels don't retry; progress counts attempts (not successes); per-file error list (`fileResults`/`failedFiles`) with a named summary instead of one last-error string. `verifyAndMoveFile` (throws→Bool, false-on-hash-mismatch) replaced by `verifyAndMoveFailure` (nil-on-success, typed failure). | `testP1_10_FailureClassification`, `testP1_10_FileResultIdentity` | `a3fa10a` |
| P1-11 | `*.ipa` + `*.ipa.sha256` added to `.gitignore`; all 8 tracked IPAs untracked (`git rm --cached`, files stay on disk). CI remains the only IPA producer (upload-artifact). | Pre-flight: `git ls-files` shows no `.ipa` | `a3fa10a` |
| P1-12 | Honest copy ("Private by Design" + optional-sync disclosure naming both GitHub hosts); `ZeroNetworkTests` rewritten from theater to three real assertions: no third-party telemetry SDKs, networking confined to the sync path, sync touches only first-party GitHub hosts. | Rewritten `ZeroNetworkTests` (3 tests) + `testP1_12_PrivacyCopyMentionsOptionalSync` | `a3fa10a` (+ repair `3dcca5d`) |

### Batch 3 red-run repairs (all green in Run 13 `35059783437`)
| Run | Failure | Fix | Commit SHA |
|-----|---------|-----|------------|
| Run 12 `35059365284` | New `ZeroNetworkTests` false positive: "Adjust" matched `.accessibilityAdjustableAction` | Drop English-word symbols; keep vendor-qualified ones (`Segment.io`, `Branch.io`) | `3dcca5d` |
| Run 13 `35059783437` | **SUCCESS** — full gate green (tests + unsigned build + IPA package) | — | `3dcca5d` |

## Fix Log (Batch 4 — P2 sweep; CI runs consumed: 22/25)
| Area | Fix summary | Test(s) | Commit SHA |
|------|-------------|---------|------------|
| Perf | `getLibraryStorageSizeBytes()` no longer runs per body evaluation: `TodayView` + `SettingsView` cache in `@State`, refreshed on appear/foreground/scan. `rescanAndVerifyLibrary` awaits `reloadCatalog()` (new awaitable wrapper) before verifying — no more stale-manifest race. | Tier-1 + CI suite | `e42f1c3` |
| Fidelity | `PlaybackResume` gains `contentType`/`dayNumber`/`videoAttachmentPath` (schema v1.2 + lightweight v1.1→v1.2 stage; `BackupPlaybackResume` carries them with tolerant decoding); `TodayView.resumeCard` rebuilds losslessly, `onSaveResume` persists them. `nextCourseSession` waiver branches collapsed (waiver lives in the catalog's omitted days). `CompletionView.init` fake defaults removed — real params required. | `P2SweepRemediationTests` (v1.2 migration round-trip, backup round-trip) | `e42f1c3` (+ schema repairs below) |
| Nits | Streak cap raised to ~150y with documented bound; countdown `Back` no-op removed; Progress calendar now opens `MonthlyHeatmapView` with real data; `MediaFilter.audio` honors on-disk presence and singles badge counts on-disk files; `MediaServicesReset` rebuilds the player; interruption end without `shouldResume` settles to `.paused`; `wantsToPlay` guards readyToPlay; artwork uses SF directly (AppIcon isn't an asset); `endReceivingRemoteControlEvents` on unregister; video controller no longer double-fullscreens; reminder hour/minute validated + toggle task cancellable; favorite failures surface inline; import size cap (256 MB), `backupVersion >= 1` gate, clean-restore goal semantics, export timestamp consistency (`exportedAt`), `appVersion` default 2.4.0; `CatalogService.loadWarning` separated from `loadError` and surfaced in Library. | `P2SweepRemediationTests` (gates, validation, schema-loudness) | `e42f1c3` |
| Dead code | Removed `deinit`-only comment in `PlaybackEngine`; one-line `@unchecked Sendable` rationales on `PersistenceErrorRelay`/`DateFormatterCache`/`HapticService`; `report()` re-hop simplified. `updateSettings` kept (public actor API, now covered by pass persistence). | — | `e42f1c3` |
| CI | Pinned `XCODEGEN_VERSION` 2.46.0 (verified latest); release `versionDate`/`date` use `date -u +%Y-%m-%d` instead of hardcoded `2026-08-21`. | Workflow YAML valid | `e42f1c3` |

### Batch 4 red-run repairs (all green in Run 22 `35068119799`)
| Run | Failure | Fix | Commit SHA |
|-----|---------|-----|------------|
| Run 14 `35063887465` | Test host crash at launch: `Duplicate version checksums` — live @Model classes registered under v1 + v1.1 + v1.2 | Per-version snapshot classes; then nested inside schema enums | `5d1d1ae`, `382e438` |
| Run 15 `35064561407` | `@Model requires an initializer` on frozen snapshots | Trivial `init() {}` on snapshots | `672086b` |
| Run 16 `35064737885` | `What kind of backing data … V1.UserSettings` — stages referenced live classes | Nest `PendingCompletion` in v1.1, `PlaybackResume` in v1.2 | `382e438` |
| Run 17 `35065220228` | Same backing-data crash via test ad-hoc `Schema([...])` containers | All test containers → `Schema(versionedSchema: v1.2)` | `b601e02` |
| Run 18 `35065770410` | Same crash via `ModelConfiguration(isStoredInMemoryOnly:)` (default live schema) | Pass versioned schema into every test configuration | `9a5c401` |
| Run 19 `35066171307` | Same crash via v1.0 snapshot enum (third owner) | Collapse baseline to v1.1 (empty-marker versions only for history) | `095e9d6` |
| Run 20 `35066860843` | Snapshot/fetch mismatch + v1.1 `PendingCompletion` backing-data crash | Snapshots only for the current version; v1.1 empty marker | `553ce9a` |
| Run 21 `35067414078` | Rewrite dropped the live `PendingCompletion` class | Restored live model; v1.1 stays an empty marker | `a14c5bb` |
| Run 22 `35068119799` | **SUCCESS** — full gate green (tests + unsigned build + IPA package) | (plus stage-count test fix `80d5753`) | `80d5753` |
